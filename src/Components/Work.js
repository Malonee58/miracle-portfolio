...
function Work({ position, company, location, type, duration }) {
	return (
		<article className="group relative pl-8 pb-10 last:pb-0 border-l border-soft last:border-transparent">
			<span className="absolute -left-[5px] top-1.5 w-2.5 h-2.5 rounded-full bg-gradient ring-4 ring-page" />

			<div className="card-surface p-5 md:p-6 transition-all duration-300 ease-premium group-hover:-translate-y-0.5 group-hover:shadow-glow">
				<div className="flex flex-wrap items-center justify-between gap-2">
					<h1 className="text-heading font-display font-semibold md:text-lg">
						{position}
					</h1>
					<span className="chip !text-indigo-accent">{type}</span>
				</div>
				...
				<div className="flex flex-wrap justify-between gap-3 pt-3">
					<div className="flex flex-col sm:flex-row sm:gap-5 gap-1">
						<div className="flex items-center gap-1.5">
							<svg
								className="fill-heading/70 shrink-0"
								width="14"
								height="12"
								viewBox="0 0 16 12"
								fill="none"
							...
							<p className="text-body text-sm">{company}</p>
						</div>
						<div className="flex items-center gap-1.5">
							<svg
								className="fill-heading/70 shrink-0"
							...
							<p className="text-body text-sm">{location}</p>
						</div>
					</div>
					<p className="font-mono-tag text-body">{duration}</p>
				</div>
			</div>
		</article>
	);
