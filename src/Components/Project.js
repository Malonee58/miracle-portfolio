...
function Project({
	...
	return (
		<article
			...
			className={`reveal group card-surface overflow-hidden transition-all duration-500 ease-premium hover:-translate-y-1.5 hover:shadow-glow ${
				featured ? "md:col-span-2 md:grid md:grid-cols-2 md:items-stretch" : ""
			}`}
		>
			<div className="relative overflow-hidden">
				{featured && (
					<span className="absolute top-4 left-4 z-10 chip !bg-surface/90 dark:!bg-surface-dark/90 !text-indigo-accent">
						★ Featured
					</span>
				)}
				<div className="aspect-[16/10] overflow-hidden">
					<img
						...
						alt={`${title} preview`}
						loading="lazy"
						className="w-full h-full object-cover transition-transform duration-700 ease-premium group-hover:scale-[1.06]"
					/>
				</div>
			...
			<div className="p-5 md:p-6 flex flex-col">
				<h1 className="text-heading font-display font-semibold text-lg">
					{title}
				</h1>
				<p className="text-body pt-3 text-sm leading-relaxed">{description}</p>
				...
				<div className="flex items-center gap-5 mt-6 pt-4 border-t border-soft">
					<a
						href={previewLink}
						target="_blank"
						rel="noreferrer noopener"
						className="flex items-center gap-1.5 text-sm font-medium text-heading hover:text-indigo-accent transition-colors"
					>
						<svg
							width="16"
							height="16"
